...
def jugar(pantalla, tablero, nombre, posicion_jugador):
    ...
    # Elegir y mostrar pregunta
        ...
        # Mover jugador (acá podés llamar a tu función mover_jugador si la tenés)
        if resultado == 1:
            posicion_jugador += tablero[posicion_jugador]
        else:
            posicion_jugador -= tablero[posicion_jugador]
...
posicion_jugador = 14
jugar(pantalla, tablero, nombre, posicion_jugador)
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            print("estoy clickeando el mouse")
    pantalla.fill(VIOLETA_VACIO)
    dibujar_tablero(posicion_jugador)
    #posicion_jugador = 14
    #jugar(pantalla, tablero, nombre, posicion_jugador)
